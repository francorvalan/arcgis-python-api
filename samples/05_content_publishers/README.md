...
- `orientation`: orientación de hoja (`"vertical"` por defecto, o `"horizontal"`).
- `order_by_field`: campo opcional para ordenar resultados antes de exportar (por defecto `None`).
- `photo_mode`: define qué adjunto(s) usar cuando hay varias fotos:
  - `"primera"`
  - `"ultima"`
  - `"random"`
  - `"all"` (por defecto)
